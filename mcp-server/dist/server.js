...
function runAdas(domain, iterations) {
...
app.post('/run-script', async (req, res) => {
    try {
        ...
        let output;
        if (mode === 'scout')
            output = await runner.executeScout(scriptPath, params ?? {});
        else if (mode === 'adas')
            output = await runner.executeADAS(scriptPath, params ?? {});
        else
            output = await runner.executeHybrid(scriptPath, params ?? {});
        res.json({ output });
    }
    catch (err) {
        res.status(400).json({ error: err.message });
